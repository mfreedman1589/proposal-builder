...
package, no network call, no paid service, and nothing platform-specific,
so it renders identically on Streamlit Cloud's Linux and on a Windows dev
box. Two things worth knowing before touching this:

- The crosswalk (`geo_resolver`'s `zip_points`) has zip CENTROIDS only,
  never boundary polygons -- no zip, county or state shape exists anywhere
  in this repo. So a group's "region" is drawn as colored dots at each
  zip's centroid, not a filled area. A convex hull would shade territory
  nobody targeted for anything but a single contiguous zip list, which is
  the uncommon case -- dishonest for the ordinary one. Geography reads
  through the natural clustering of the points themselves.
- No basemap or state outlines, for the same reason: there is no boundary
  data anywhere in this repo to draw them from, and fetching a new dataset
  is out of scope for "render locally from the zip centroids already in
  the crosswalk." The projection auto-fits to whatever is actually being
  plotted (never a fixed CONUS frame), so a single-region group still
  fills the frame instead of sitting as a speck on an empty continent.
"""
import math
...
def _projector(lats, lons, width, height):
    ...
    margin, both axes at the SAME scale so a tight north-south group isn't
    stretched wide to fill the frame -- it's centered in the slack instead.
    """
    ...
    def project(lat, lon):
        x = (lon * cos_lat - min_x) * scale + offset_x
        y = (-lat - min_y) * scale + offset_y
        return x, y

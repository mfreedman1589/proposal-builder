...
uploaded as a file (not just pasted), a Wide Orbit export uploaded before
Total TV is even on, and the client logo from its new location -- through
AppTest + test_mode_upload injection, the same technique
test_avails_pdf_wiring.py and test_wideorbit.py use for a file_uploader,
which AppTest can't drive directly.
...
def main():
    ...
    print("\n" + "=" * 78)
    print("SCENARIO  Wide Orbit uploaded at intake, before Total TV is even on")
    print("=" * 78)
    if not WO_FIXTURE.exists():
        ...
        check("the schedule parsed and landed in session_state before Total TV was ever touched",
              ss(at3, "broadcast_schedule") is not None, ss(at3, "wo_error"))
        check("the intake area itself confirms it loaded",
              "Loaded" in all_text(at3.caption), None)

        print("\nturning Total TV on shows the ALREADY-parsed summary, not an empty panel "
              "or a re-upload prompt")
        ...
        check("no \"upload it up top\" pointer shown -- it's already there",
              "No Wide Orbit schedule uploaded yet" not in all_text(at3.caption), None)

        print("\nwithout ANY upload, Total TV points back at the intake area instead of an "
              "empty metrics panel")
        at4 = new_app()
        at4.session_state["total_tv"] = True
        at4.run()
        check("no exception", not at4.exception, at4.exception[0].message[:400] if at4.exception else "")
        check("the pointer-back caption is shown",
              "No Wide Orbit schedule uploaded yet" in all_text(at4.caption), None)
        check("no metrics panel -- nothing to show yet",
              not any(getattr(m, "label", None) == "Spots" for m in at4.metric), None)
    ...
    if failures:
        ...
        return 1
    print("The intake area's three new/relocated pieces all reach the same state a rep "
          "typing or uploading directly into the old locations would have: notes-file "
          "upload seeds the paste box once and never overwrites an edit; a Wide Orbit "
          "export uploaded before Total TV is on is already parsed and waiting once it is; "
          "the logo uploader works identically from its new home.")
    return 0
